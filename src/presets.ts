...
export type PresetFn = (args: PresetArgs) => Partial<GridItemProps>[];
...
// Tile tiers (and the strict-axis flex allowance), as a fraction of `nrCols` rather than an
// absolute cell count, so the preset scales to any `nrCols` instead of one tuned column count.
...
export const organicPreset =
  ...
  ({ count, nrCols }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let tile: Partial<GridItemProps>;
      if (mode < 0.3) {
