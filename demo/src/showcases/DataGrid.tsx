...
export function DataGrid({
  data,
  ...overrides
}: { data: ReportCase } & Omit<GridProps, "children">) {
  // same rule as the other cases: no caption in a cell too small to hold one
  const hasLabels =
    typeof overrides.rowHeight !== "number" || overrides.rowHeight >= 44;
  return (
    <Grid
      animateSize
    ...
    >
      {data.tiles.map((tile, i) => {
        ...
        const spec = [
          ...
          item.cols === undefined || item.rows === undefined
            ? `w${item.weight ?? 1}`
            : null,
        ]
          .filter(Boolean)
        ...
        return (
          ...
          <GridItem key={i} {...item}>
            {kind === "void" ? (
              ...
              <Tile n={i} label={hasLabels ? spec : undefined} />
            )}
          </GridItem>
        );
      })}
