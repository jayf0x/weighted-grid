...
/** Short, collision-free abbreviations for `Item`'s dev-mode debug labels — `stretch`/`stretchX`/
 * `stretchY` all share a first letter, so a naive `k.slice(0,1)` can't tell them apart. Ported
 * verbatim from `dev/src/lib/debugLabel.ts`. */
