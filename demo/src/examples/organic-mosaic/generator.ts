...
//
// Ported from dev/src/cases/organic.ts, unchanged, onto demo's `ExampleTile` type.
...
const voidThinFor = (nrCols: number) => Math.max(2, Math.min(3, Math.round(nrCols / 16)));
...
export function generateOrganicTiles(seed = 1, count = 20, nrCols = 30): ExampleTile[] {
  ...
  const areaOf = (t: Partial<Pick<ExampleTile, 'cols' | 'rows' | 'weight'>>) =>
  ...
  for (let i = 0; i < count; i++) {
    ...
    const wantsVoid =
      voidFrac < TARGET_VOID_AREA_FRAC && ((sinceVoid >= 2 && urge > 0.45) || sinceVoid >= 5);

    if (wantsVoid) {
      const vertical = pick(i * 5.9) < 0.5;
      const longWeight =
        VOID_WEIGHT_TIERS[Math.floor(pick(i * 6.7) * VOID_WEIGHT_TIERS.length) % VOID_WEIGHT_TIERS.length];
      const shape = vertical ? { cols: voidThin, weight: longWeight } : { rows: voidThin, weight: longWeight };
